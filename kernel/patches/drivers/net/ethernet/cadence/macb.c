...
static int macb_mii_probe(struct net_device *dev)
{
	...
	/* MM attach PHY to MAC using the PHY handle property */
	if (bp->phy_node) {
			phydev = of_phy_connect(bp->dev,
						...
						0,
						bp->phy_interface);
			netdev_info(dev, "PHY@%u connected with %s", phydev->addr, dev->name);
	}

	/* If no matching PHY was found on the bus, use the first one */
	if (!phydev) {
...
static int __init macb_probe(struct platform_device *pdev)
{
	...
	/* MM: Here find out the PHY handle */
	bp->phy_node = of_parse_phandle(np, "phy-handle", 0);

	if (!bp->phy_node) {
		dev_err(&pdev->dev, "Failed parsing PHY handle %u");
		/* MM TODO: Error handling */
	}

	else {
		dev_info(&pdev->dev, "Found PHY handle %u", bp->phy_node->phandle);
	}
