...
/* 88E1812 PHY defines */
#define MIIM_88E1812_PHY_PAGE_REG			22
...
static int m88e1512_config(struct phy_device *phydev)
{
	...
	/* Set PHY LED0 function On-Link, Off-No link, Blink-Activity */
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E1812_PHY_PAGE_REG, 3);
	reg = phy_read(phydev, MDIO_DEVAD_NONE, 16);
	reg &=~0x000f;
	reg |= 0x0001;
	phy_write(phydev,MDIO_DEVAD_NONE, 16, reg);
	/***************************************/
	/* Set PHY Mode to RGMII to 1000Base-X */
	/**************************************/
	/* General control register is at 20 Page 18 */
	//printf("Set PHY mode to RGMII to 1000Base-X, MODE[010]\n");
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E1812_PHY_PAGE_REG, 18);
	reg = phy_read(phydev, MDIO_DEVAD_NONE, 20);

	/* Set MODE[2:0] to "010" (RGMII (System Mode) to 1000Base-X) */
	reg &=~0x0007;
	reg |= 0x0002;
	phy_write(phydev, MDIO_DEVAD_NONE, 20, reg);
	reg = 0;

	/* Perform a soft reset of Page 6 and 18 */
	//printf("Reset PHY mode state machine (Page 6 & 18)\n");
	reg = phy_read(phydev, MDIO_DEVAD_NONE, 20);
	reg |= 0x8000;
	phy_write(phydev, MDIO_DEVAD_NONE, 20, reg);
	reg = 0;
	...
	/***************************************/
	/* Set SERDES output amplitude */
	/***************************************/
	/* Set the SGMII/SERDES Output amplitude to 504 mV */
	//printf("Set SGMII output amplitude to 504mv\n");
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E1812_PHY_PAGE_REG, 1);
	...
	/***************************************/
	/* Page 2 / Register 21 */
	//printf("Set RGMII RX/TX delay\n");
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E1812_PHY_PAGE_REG, 2);
	reg = phy_read(phydev, MDIO_DEVAD_NONE, 21);
	reg |= 0x0030;
	phy_write(phydev, MDIO_DEVAD_NONE, 21, reg);
	reg = 0;

	/* Referring to Rev A0 and Release Notes for 88E1510/12/14/18
	 * 3.3 auto-negotiation advertisement register needs to be reprogrammed
	 * when changing mode to 1000Base-X.  */
	//printf("Set auto-negotiation advertisement register (As per release note 3.3)\n");
	phy_write(phydev, MDIO_DEVAD_NONE, MIIM_88E1812_PHY_PAGE_REG, 1);
	phy_write(phydev, MDIO_DEVAD_NONE, 4, 0x0060);
...
static int m88e1512_shutdown(struct phy_device *phydev)
{
	return 0;
}
