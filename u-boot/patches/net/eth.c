...
static void eth_env_init(bd_t *bis)
{
	...
	/* Import ethernet environment variables from shared RAM */
	puts("\n");
	cli_simple_run_command("env import -t 0xFFFFFC00", 0);
	cli_simple_run_command("env import -t 0xFFFFFD00", 0);
	puts("\n");

	cli_simple_run_command("printenv ethaddr", 0);
	cli_simple_run_command("printenv eth1addr", 0);
